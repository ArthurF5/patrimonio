...
class Relatorios_model extends CI_Model
{
    ...
    public function movimentacoesRapid()
    {
        ...
        // from
        //     bens b
        // inner join protocolo p on p.bem_id = b.id

        // order by
        //     b.id asc,
        //     p.data asc
        $this->db->select('b.id, b.descricao, p.protocolo, p.tipo, p.data, p.historico');
        $this->db->from('bens b');
        $this->db->join('protocolo p', 'p.bem_id = b.id', 'inner');
        $this->db->order_by('b.id', 'asc');
        $this->db->order_by('p.data', 'asc');
